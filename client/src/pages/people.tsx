    ...
    <div className="flex flex-col h-full bg-[#F5F2EE]">
      ...
      <div className="px-5 pt-12 pb-4">
        <div className="flex items-end justify-between mb-1">
          <h1 className="font-serif text-[28px] font-medium text-[#1A1A1A] leading-none">People</h1>
      ...
      <div className="flex-1 overflow-y-auto hide-scrollbar pb-nav">
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-48 text-[#1A1A1A]/20">
            <Users className="w-8 h-8 mb-3" strokeWidth={1} />
          ...
          <motion.ul>
            {filtered.map((item) => (
              ...
              >
                {item.photoUrl ? (
                  <img src={item.photoUrl} alt={item.name} className="w-11 h-14 object-cover flex-shrink-0 object-top bg-black/5" />
                ) : (
                  <div className="w-11 h-14 bg-black/5 flex-shrink-0" />
                )}
                <div className="flex-1 min-w-0 pt-0.5">
                  <p className="text-[15px] font-serif font-medium text-[#1A1A1A] leading-snug">{item.name}</p>
                  {item.knownFor && (
                    <p className="text-[11px] text-[#1A1A1A]/40 mt-0.5">{item.knownFor}</p>
                  )}
                  {item.knownForTitles.length > 0 && (
                    <p className="text-[11px] text-[#1A1A1A]/30 mt-0.5 truncate">{item.knownForTitles.slice(0, 3).join(", ")}</p>
                  )}
                </div>
              </motion.li>
            ))}
                ...
                >
                  {r.profile_path ? (
                    <img
                      ...
                      alt=""
                      className="w-9 h-12 object-cover object-top flex-shrink-0 bg-black/5"
                    />
                  ) : (
                    <div className="w-9 h-12 bg-black/5 flex-shrink-0" />
                  )}
                  <div className="flex-1 min-w-0">
                    <p className="text-[15px] font-serif font-medium text-[#1A1A1A] leading-snug">{r.name}</p>
                    <p className="text-[11px] text-[#1A1A1A]/40 mt-0.5">{r.known_for_department}</p>
                    <p className="text-[11px] text-[#1A1A1A]/30 mt-0.5 truncate">
