...
export default function Movies() {
  ...
  return (
    <div className="flex flex-col h-full bg-[#F5F2EE]">
                ...
                {!urlMode ? (
                  ...
                  >
                    ...
                    {isSearching ? (
                      ...
                      <button onClick={closeSearch} className="text-[10px] uppercase tracking-[0.2em] text-[#1A1A1A]/35 font-medium">
                        Cancel
                      </button>
                    )}
                  </motion.div>
            ...
            <div className="flex-1 overflow-y-auto hide-scrollbar">
              ...
              {urlMode && (
                <div className="px-5 pt-6">
                  {urlError && (
                    <p className="text-[12px] text-[#8B2635] mb-4">{urlError}</p>
                  )}
                  ...
                  <p className="text-[11px] text-[#1A1A1A]/20 mt-3 font-light">
                    e.g. themoviedb.org/movie/27205-inception
                  </p>
                  <button
                    onClick={() => { setUrlMode(false); setTimeout(() => inputRef.current?.focus(), 50); }}
                ...
                <>
                  {loading && (
                    <div className="flex justify-center py-14">
                      <Loader2 className="w-5 h-5 animate-spin text-[#1A1A1A]/30" strokeWidth={1.5} />
                    </div>
                  )}
                  ...
                  {/* Can't find it prompt — shown when there's a query but no results */}
                  {!loading && query.trim().length > 1 && (
                    <div className="px-5 pt-6 pb-4">
                      <button
                        onClick={() => { setUrlMode(true); setTimeout(() => urlInputRef.current?.focus(), 50); }}
      ...
      <AnimatePresence>
        {selectedItem && (
          <DetailView item={selectedItem} type="movie" onClose={() => setSelectedItem(null)} onDelete={remove} />
        )}
      </AnimatePresence>
    </div>
  );
